        ...
        def _decorator(decorated_function):
            ...
            def _wrapper(*args, **kwargs):
                ...
                if DirectiveMeta._when_constraints_from_context:
                    ...
                    when_constraints = [
                        Spec(x) for x in DirectiveMeta._when_constraints_from_context
                    ]
                    if kwargs.get("when"):
                        when_constraints.append(Spec(kwargs["when"]))
                    # FIXME:
                    when_spec = merge_abstract_anonymous_specs(*when_constraints)
...
def variant(
    ...
    def format_error(msg, pkg):
        msg += " @*r{{[{0}, variant '{1}']}}"
        return colorize(msg.format(pkg.name, name))
